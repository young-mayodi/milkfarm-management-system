class SalesRecord < ApplicationRecord
  ...
  def self.cost_breakdown_analysis(farm, date_range = 1.month.ago..Date.current)
    ...
    {
      feed_costs: (production_volume * 25.0).round(2),
      labor_costs: (((date_range.end - date_range.begin).to_i + 1) * 500.0).round(2),
      veterinary_costs: (production_volume * 3.0).round(2),
      maintenance_costs: (production_volume * 2.0).round(2),
      total_estimated_costs: (production_volume * 30.0 + ((date_range.end - date_range.begin).to_i + 1) * 500.0).round(2)
    }
  end
