...
class RecordModulesTest
  ...
  def check_response(response, test_name)
    ...
    when 500
      ...
      return { status: :failed, message: "500 Internal Server Error" }
    when 404
      puts "⚠️  #{test_name}: Not Found (404)"
      return { status: :not_found, message: "404 Not Found" }
    when 302, 301
      puts "↗️  #{test_name}: Redirect (#{response[:code]})"
      return { status: :redirect, message: "Redirect #{response[:code]}" }
    else
      puts "⚠️  #{test_name}: Unexpected status (#{response[:code]})"
      return { status: :unexpected, message: "Status #{response[:code]}" }
    end
  end
